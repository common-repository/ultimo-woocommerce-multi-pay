...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.8.3', __( 'WooCommerce Authorize.Net AIM Gateway', 'ultimo-woomultipay' ), __FILE__, 'init_woocommerce_gateway_authorize_net_aim', array(
...
class WC_Authorize_Net_AIM extends SV_WC_Payment_Gateway_Plugin {
	...
	public function plugin_action_links( $actions ) {
		...
		$title  = $this->is_emulation_enabled()
			? esc_html__( 'Disable Emulation Gateway', 'ultimo-woomultipay' )
			: esc_html__( 'Enable Emulation Gateway', 'ultimo-woomultipay' );
	...
	public function get_settings_link( $gateway_id = null ) {
		...
		switch ( $gateway_id ) {
			...
			case self::EMULATION_GATEWAY_ID:
				$label = __( 'Configure Emulator', 'ultimo-woomultipay' );
			break;

			case self::ECHECK_GATEWAY_ID:
				$label = __( 'Configure eChecks', 'ultimo-woomultipay' );
			break;

			default:
				$label = __( 'Configure Credit Cards', 'ultimo-woomultipay' );
		}
	...
	public function add_admin_notices() {
		...
		if ( $credit_card_gateway->is_enabled() && $credit_card_gateway->is_accept_js_enabled() && isset( $_GET['page'] ) && 'wc-settings' === $_GET['page'] ) {
			...
			if ( ! $credit_card_gateway->get_client_key() ) {
				$message = sprintf( __( "%s: A valid Client Key is required to use Accept.js at checkout.", 'ultimo-woomultipay' ), '<strong>' . $this->get_plugin_name() . '</strong>' );
			} elseif ( ! wc_checkout_is_https() ) {
				$message = sprintf( __( "%s: SSL is required to use Accept.js at checkout.", 'ultimo-woomultipay' ), '<strong>' . $this->get_plugin_name() . '</strong>' );
			}
		...
		if ( ! empty( $_GET['wc_authorize_net_aim_emulation'] ) ) {
			...
			$message = ( 'enable' === $_GET['wc_authorize_net_aim_emulation'] )
				? __( 'Authorize.Net AIM Emulation Gateway is now enabled.', 'ultimo-woomultipay' )
				: __( 'Authorize.Net AIM Emulation Gateway is now disabled.', 'ultimo-woomultipay');
	...
	public function get_plugin_name() {
		return __( 'WooCommerce Authorize.Net AIM Gateway', 'ultimo-woomultipay' );
	}
	...
	/**
	 * Returns __FILE__
	 * UWOOMP fix: change method to public
	 *
	 * @since 3.0
