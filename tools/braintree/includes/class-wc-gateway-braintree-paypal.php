...
class WC_Gateway_Braintree_PayPal extends WC_Gateway_Braintree {
	...
	public function __construct() {
		...
		parent::__construct(
			...
			wc_braintree(),
			array(
				'method_title'       => __( 'Braintree (PayPal)', 'ultimo-woomultipay' ),
				'method_description' => __( 'Allow customers to securely pay using their PayPal account via Braintree.', 'ultimo-woomultipay' ),
				'supports'           => array(
	...
	public function adjust_token_editor_fields() {
		...
		$fields = array(
			...
			'payer_email' => array(
				'label'   => __( 'Email', 'ultimo-woomultipay' ),
				'editable' => false,
			),
		);
	...
	public function tweak_payment_methods_text( $translated_text, $raw_text, $text_domain ) {
		...
		if ( 'woocommerce-plugin-framework' === $text_domain ) {
			...
			if ( 'Use a new bank account' === $raw_text ) {
				...
				$translated_text = __( 'Use a new PayPal account', 'ultimo-woomultipay' );

			} elseif ( 'Bank Accounts' === $raw_text ) {

				$translated_text = __( 'PayPal Accounts', 'ultimo-woomultipay' );
			}
		}
	...
	public function tweak_my_payment_methods_delete_text( $actions, $token ) {
		...
		if ( $token->is_paypal_account() ) {
			$actions['delete']['name'] = __( 'Unlink', 'ultimo-woomultipay' );
		}
	...
	protected function do_paypal_transaction( WC_Order $order ) {
		...
		if ( $response->transaction_approved() ) {
			...
			$message = sprintf(
				__( '%1$s %2$s %3$s Approved', 'ultimo-woomultipay' ),
				$this->get_method_title(),
				$this->is_test_environment() ? __( 'Sandbox', 'ultimo-woomultipay' ) : '',
				$this->perform_credit_card_authorization() ? __( 'Authorization', 'ultimo-woomultipay' ) : __( 'Payment', 'ultimo-woomultipay' )
			);

			// adds the transaction id (if any) to the order note
			if ( $response->get_transaction_id() ) {
				/* translators: Placeholders: %s - transaction ID */
				$message .= ' ' . sprintf( __( '(Transaction ID %s)', 'ultimo-woomultipay' ), $response->get_transaction_id() );
			}
	...
	protected function get_saved_payment_token_order_note( $token ) {
		...
		return sprintf( __( 'PayPal Account Saved: %s', 'ultimo-woomultipay' ), $token->get_payer_email() );
	}
	...
	protected function get_default_title() {
		...
		return __( 'PayPal', 'ultimo-woomultipay' );
	}
	...
	protected function get_default_description() {
		...
		return __( 'Click the PayPal icon below to sign into your PayPal account and pay securely.', 'ultimo-woomultipay' );
	}
